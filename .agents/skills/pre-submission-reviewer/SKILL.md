...
description: >-
  Audits a complete mathematical-modeling contest paper before submission:
  problem coverage, model-to-result logic, evidence and citation discipline,
  language quality, LaTeX/template integrity, figures, and AI-tone leakage.
  Use only for a coordinator-assigned read-only staged gate or final full-paper
  review after Writer has produced the required LaTeX tree and PDF. Reports
  CRITICAL, MAJOR, and MINOR findings with concrete evidence and rework owners.
---
...
- Follow the Reviewer role, `report-writing`, `result-validation`,
  `self-audit-loop`, and exactly one matching contest-template skill. Those
  rules own the final PASS/FAIL gate and take precedence over generic venue
  advice in this skill.
- Review only the coordinator-supplied work name and paths. Remain read-only,
  do not spawn agents, and route every repair through the Coordinator to
  Modeler, Coder, or Writer.
- For Chinese CUMCM work, review Chinese grammar, terminology, and naturalness;
  do not apply English-only article, tense, or em-dash rules. For English
  MCM/ICM work, apply the English grammar rules.
- A final PASS also requires the selected template and language, required file
  tree, compiled PDF, body-page policy, numerical traceability, path-leakage
  scan, and core-code appendix evidence defined by the Reviewer role.
...
- The paper is still being structured or its model/result chain is unsettled.
  Return it to the Coordinator for a bounded Modeler, Coder, or Writer stage.
...
**Contest fit**: verify that the submission answers every stated problem and
uses the assigned contest family, language, template, and deliverable format.
Treat any mismatch as a blocking user-goal mismatch under the Reviewer role.

...
- **Rung 2, same-context grounding.** No sub-agents, but web search or other
  retrieval is available. Re-derive every reference entry from actual
  retrieval responses, never from memory, and check each claim against what
  retrieval returned. Weaker than rung 1; say so.

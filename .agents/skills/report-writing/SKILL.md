...
## Local workflow integration boundary

When this skill is used in this workspace, the Coordinator/Writer prompts and
the selected `cumcm-writing` or `mcm-writing` skill are authoritative for
assignment scope, template, language, report tree, page policy, compilation,
and completion. This skill supplies claim discipline and evidence organization;
it does not merge Writer stages, shorten a required chapter, or treat a
compiling PDF as complete. If evidence is thin, record the gap and route a
bounded Modeler/Coder expansion instead of silently deleting required report
content.
...
- Do not expose local file paths, workspace addresses, template source paths, raw artifact filenames, shell commands, tool/runtime details, or agent handoff language in the reader-facing report. Keep those details in an internal README/status artifact when needed; use semantic labels, captions, and artifact descriptions in the report itself.

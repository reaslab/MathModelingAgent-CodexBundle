...
name: paper-polish
description: >-
  Polishes existing mathematical-modeling paper prose while preserving the
  accepted model, equations, numerical results, citations, and conclusion
  strength. Use for a coordinator-assigned language-polish or translation pass
  on one existing Writer artifact: repair grammar and flow, remove mechanical
  AI tone, stabilize terminology, and calibrate claims without inventing or
  altering technical content.
---
...
- Apply this skill only after the target section has accepted modeling and
  numerical evidence. Polish exactly the Writer artifact named by the
  Coordinator and preserve all LaTeX commands, labels, citations, equations,
  units, identifiers, and reported values unless verified evidence requires a
  correction.
- Keep the assigned contest language. Chinese CUMCM prose stays natural,
  concise Chinese; English MCM/ICM prose stays idiomatic academic English. Do
  not translate unless translation is the explicit target.
- Edit the assigned file directly, compile through the Writer workflow, and
  record meaning-risk items in `writer/README.md`. Do not leave editorial notes
  or placeholders inside the manuscript.
- Do not spawn agents or route work directly. Return structural, modeling, or
  result defects to the Coordinator with concrete evidence.
...
- No prose exists yet. Use `paper-writer` after Modeler/Coder evidence is ready.
...
If a passage genuinely lacks evidence or argument, **do not invent it** and do
not leave a placeholder or editorial note in the paper. Record the exact gap in
the Writer status and return it to the Coordinator.
...
If the text needs a structural rewrite beyond the assigned language pass, stop
and return a bounded rework request to the Coordinator. Do not silently expand
the scope.
...
2. **Find the real problem.** Is it surface grammar and wording, or is the
   ...
   redrafting rather than polishing (return a Writer rework request).
...
1. **The text is beyond sentence repair.** A broken logic chain, a Discussion
   ...
   a separate `paper-writer` assignment, then return to polish.
2. **The core claim itself may not hold.** For example, the author's own
   ...
   plainly and request Coordinator-routed Modeler/Coder/Reviewer audit before
   further polishing.

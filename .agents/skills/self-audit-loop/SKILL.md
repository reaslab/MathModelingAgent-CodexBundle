...
In this workspace this is an audit layer, not a replacement for the Coordinator's staged
workflow or the contest writing contract. Return risks, claim downgrades, and
the exact upstream evidence or repair target needed; do not collapse a report
to the shortest defensible version when the native page policy requires more
evidence-backed derivation, validation, comparison, sensitivity, or limitation
analysis. A missing fact must be verified or routed for repair, never invented;
an audit finding must not by itself mark the paper complete.
